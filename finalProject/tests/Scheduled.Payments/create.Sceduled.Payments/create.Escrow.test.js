...
// const ModalConfirm = require('../pageobjects/pageComponents/modalForms/modalFormConfirm');
...
const deductBy = ModalFormNewPayment.deductBy;
const category = constants.CATEGORY_ESCROW;

let lastPaymentID;
let newPaymentID;


describe ('Creation a new Scheduled ESCROW ',  async () => {
  before(async () => {
  ...
  it(`User cah create new ESCROW with driver, category, amount and deduct by`, async () => {
    ...
    await ModalFormNewPayment.chooseRadioEscrow(); // work only here!!! do not move!!!

    const inputAmount = await ModalFormNewPayment.fillInputAmount(amount);
    const inputDeduct =  await ModalFormNewPayment.fillInputDeductBy(deductBy);
