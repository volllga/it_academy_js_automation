...
class PageLoginObject {
  ...
  get btnSubmit () {return $('#login-form__login-button')}
  get btnSingIn(){return $('.c-button-save')}
  get inputEmail () {return $('#login-form__email')}
  get inputPassword () {return $('#login-form__password')}
  get feedbackEmail() {return $('#login-form__email-feedback')}
  get feedbackPassword() {return $('#login-form__password-feedback')}
  get feedbackLoginForm() {return $('h5')}

  openLoginPage () {return browser.url(this.loginUrl)}
  ...
  async login () {
    ...
    await (PageLoadsObject.userName).waitForDisplayed({ timeout: 200000 })
      .then(await expect(PageLoadsObject.userName).toHaveTextContaining(this.user));
  }
