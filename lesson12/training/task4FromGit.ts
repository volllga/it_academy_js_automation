 ...
 * @return {Array | Function}
 */
type filtererType<T> = (value: T, index: number, arr: T[]) => T[];
export function filter<T>(filterer: filtererType<T>, input: Array<T>): Array<T> | Function {
    if (arguments.length === 0) {
        return filter;
    }
    if (arguments.length === 1) {
 ...
 * @return {* | Function}
 */
type reducerType<T> = (accumulator: T, currentValue: T, currentIndex: number, sourceArray: Array<T>) => T;
export function reduce<T>(reducer: reducerType<T>, initialValue: T, input: Array<T>): T | Function {
    if (arguments.length === 0) {
        return reduce;
    }
    if (arguments.length === 1) {
        return function subFunction(subInitialValue: T, subInput: Array<T>) {
            if (arguments.length === 0) {
                return subFunction;
            }
            if (arguments.length === 1) {
...
export function prop(obj: Obj, propName: string): any | Function {
    if (arguments.length === 0) {
        return prop;
    }
    if (arguments.length === 1) {
